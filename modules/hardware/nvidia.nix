...

{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (config.modules.hardware.nvidia.enable && config.modules.hardware.nvidia.useNixosHardware) {
      imports = [
    ...
    (lib.mkIf config.modules.hardware.nvidia.enable {
      ...
      environment.variables = {
        LIBVA_DRIVER_NAME = "nvidia";  # Use NVIDIA for hardware video decode
