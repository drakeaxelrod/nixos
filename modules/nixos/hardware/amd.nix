...
{
  # Always import nixos-hardware AMD modules (they're harmless if AMD hardware isn't present)
  imports = [
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf config.modules.hardware.amd.gpu.enable {
      ...
      # VA-API driver (can be overridden by NVIDIA module in hybrid systems)
