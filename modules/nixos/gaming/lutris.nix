# Lutris - Game manager for Wine/Proton games with optimizations
{ config, lib, pkgs, ... }:
...
in
{
  ...
  options.modules.gaming.lutris = {
    enable = lib.mkEnableOption "Lutris game manager";

    wine = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Include Wine Staging";
    };

    winetricks = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Include Winetricks";
    };
